...
function [output] = meander_model_AR_copy(s,iR,AR,q,h,W,Cfe,slope,zb,asR,dt,modtyp,FasRb,amplitude,strongcurvature,Ashld,Bshld,approxlev)
...
if nargin < 18, approxlev = 100; end
...
b         = 5;               % Non-linearity of sediment transport
g         = 9.81;       % Gravity
...
%Cf0       = physpar.Cf0;
specden   = 2.65;            % specific density of qtz
relden    = specden-1;            % relative density
ks        = 0.0004*3; % Nikuradse roughness height
z0        = ks/30;
D50       = ks/3;
Cf0       = Cfe(:);
thet0     = Cf0.*(Q0./H0).^2./g./relden./D50;

Dst    = D50*((specden-1)*g/nukin^2)^(1/3);
thetcr = 0.055;
thetcr(Dst<=150) = 0.013*Dst^(0.29);
thetcr(Dst<=20)  = 0.04*Dst^(-0.10);
thetcr(Dst<=10)  = 0.14*Dst^(-0.64);
thetcr(Dst<=4)   = 0.24*Dst^(-1);
