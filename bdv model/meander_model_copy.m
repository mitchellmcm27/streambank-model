function [output] = meander_model_copy(s,iR,AR,q,h,W,Cfe,slope,zb,asR,dt,modtyp,FasRb,bedtype)
...
%% User settings
%[physpar] = getphyspar(data,44);
g         = 9.81;       % Gravity
karman    = 0.4;  % von Karman constant
Sn        = 1;      % Transverse slope
chi       = 1.5;     % ??
Eu        = 0;
iS        = slope;      % Valley slope
%Cf0       = physpar.Cf0;
ks        = 0.0004*3; % Nikuradse roughness height 3*D84
z0        = ks/30;
L         = 12500;%7*3.2138;        % Valley length

H0        = mean(h);
Q0        = mean(q);      %Unit discharge
Fr        = Q0/H0/sqrt(g*H0);
%Gthet     = physpar.Gthet;
%[numpar]  = getnumpar();
CFL       = 0.2;
smoothrad = 2;            % 0 = adaptation by Johannesson & Parker (1989)
                          % 1 = diffusion of geometric radius.
                          % 2 = relaxation equation with
                          
streamcurv= 1;            % 0 = none; streamline curvature at center channel
                          % 1 = according to (BdV 2009, eq (41)). <-- BdV
                          % 2010, eqn 40
%bedtype   = 1;            % 0 = flat bed   (A=0)
                          % 1 = river bed  (A=3)
                          % 2 = A/R from m89 experiment bed level)
                          % 3 = A/R from input
asRacc    = 5e-9;         % accuracy cutoff for alpha_s/R calculation
routyp    = 0;            % 0 = straight channel friction factor, Cf 
                          % 1 = calculated from grain roughness, Cf doesn't matter
%ds        = numpar.ds;
rn        = 0.99;                  % relaxation factor bewteen two alpha_s/R iterations

maxK      = 2000;              % maximum iterations of \alpha_s calculation
sweacc    = 1e-7;              % accuracy cutoff for SWE calculation
...
%     disp([num2str(k),': ',num2str(asRdiff)])
    ...
    if (streamcurv == 1)
        ...
        iRs = iR + ((asR + iR).*(W.^2/8.*((dsasR+Sn*Fr^2*dsiR+dsAR))).^2 + W.^2/8.*(matprodsol(B4a,B4b,B4c,asR+Sn*Fr^2*iR+AR)));
        dsiRs = matprodsol(B2a,B2b,B2c,iRs);
    end
end
