            ...
            for y in range(starting_year, ending_year+1):
                ...
                for m in range(m1, m2+1):
                    ...
                    if d1 == 1 and d2 == 31:
                        ...
                        raster = getMonthlyRaster(y+2012, m)
                        sum_month = arcpy.Raster(raster)
                        days = 31
                        arcpy.env.workspace =\
                            'K:/GIS/MODEL/input/PRISM/tmean'
                        traster = getMonthlyRaster(y+2012, m)
                        traster = arcpy.Raster(traster)
                    else:
                        ...
                        for d in range(d1, d2 + 1):
                            if RastersSorted[y][m][d] != '':
                                days += 1
                                RastersMonth.append(RastersSorted[y][m][d])

                        # Still in month loop...
                        # Loop through rasters between dates adding them to a
                        # monthly raster
                        RastersMonth = filter(None, RastersMonth)
                        i = 0
                        for raster in RastersMonth:
                    ...
                    table = 'K:/GIS/MODEL/output/tables/' + site_id + '_precip'
                    ttable = 'K:/GIS/MODEL/output/tables/' + site_id + '_temp'
                    ...
                    with arcpy.da.SearchCursor(table, lst_flds) as cur:
                        for row in cur:
                            ...
                            for fld in lst_flds:
                                ...
                                dct_site[key] = row[k]
                                k += 1
                    arcpy.sa.ZonalStatisticsAsTable( layer, 'BasinID', traster, ttable, 'NODATA', 'ALL')
                    with arcpy.da.SearchCursor(ttable, lst_flds) as cur:
                        for row in cur:
                            k = 0
                            for fld in lst_flds:
                                field = fld + '_%02d_tmean' % m # e.g. 'SUM_01_tmean' for january
                                if field not in Fields:
                                    Fields.append(field)
                                key = (site_id, field) # e.g. ('PR01Y1', 'SUM_01')
                                dct_site[key] = row[k]
