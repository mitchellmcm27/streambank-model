...
%  - added unit string
    ...
    if ~isempty(strfind(direction,'north'))
        ...
        textalignment = {'bottom', 'right'}; % changed center to right
    else
        ends(2,2,:) = ends(2,2,:)+axesdir(2)*0.1*scalelength;
        textalignment = {'top', 'right'}; % changed center to right
    end
    ...
    % took out line to write '0' on scale bar...
    ...
    text(linepos(2,1),linepos(2,2),0,[num2str(scalelength) unitstring],'verticalalignment',textalignment{1},'horizontalalignment',textalignment{2}, 'color', colour, 'fontweight', fontweight, 'fontsize',9, 'parent', hg);
